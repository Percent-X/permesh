...
Tài liệu này là quy chuẩn bắt buộc (Strict Rules) cho toàn bộ giao diện và mã nguồn của nền tảng **Skool Space / Permesh Community Hub**. Mọi chỉnh sửa và tính năng mới bắt buộc phải tuân thủ 100% các nguyên tắc sau:
...
* Hỗ trợ đầy đủ: In đậm `**text**`, in nghiêng `*text*`, danh sách số `1.`, bullet point `-`, trích dẫn `> quote`, link `[title](url)`, khối mã nguồn ` ``` ` và inline code ` `code` `.

---

## 7. Quy Chuẩn Xem Chi Tiết Q&A (Q&A Modal View Standard)
* Thẻ Q&A trong danh sách giữ kích thước gọn gàng, trích dẫn ngắn (`line-clamp-2`). Bấm vào bất kỳ đâu trên thẻ sẽ mở `QADetailModal` (popup nền trong suốt sạch sẽ không làm tối hay mờ trang sau, cuộn độc lập, ô nhập trả lời ghim đáy). Giữ nguyên vị trí cuộn của trang danh sách bên dưới khi đóng modal.
